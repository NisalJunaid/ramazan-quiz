<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="flex items-center justify-end mt-4">
            <a class="underline text-sm text-muted hover:text-theme rounded-md focus:outline-none" href="{{ route('login') }}">
                {{ text('auth.register.already', 'Already registered?') }}
            </a>

            <x-primary-button class="ms-4">
                {{ text('auth.register.submit', 'Register') }}
            </x-primary-button>
        </div>
    </form>
</x-guest-layout>

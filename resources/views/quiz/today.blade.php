...
@section('content')
    <div class="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 sm:px-6 lg:px-8">
        <header class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Today's Quiz</h1>
                <p class="mt-1 text-sm text-gray-600">Complete one attempt before the window closes.</p>
            </div>
            <a class="text-sm font-semibold text-indigo-600 hover:text-indigo-700" href="{{ route('leaderboard') }}">View leaderboard</a>
        </header>
        ...
        @else
            <section class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-indigo-500">Quiz Window</p>
                ...
                @if (!$attempt)
                    <form class="mt-6" method="POST" action="{{ route('quiz.start', $quizDay) }}">
                        @csrf
                        <button class="inline-flex items-center justify-center rounded-full bg-indigo-600 px-6 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700" type="submit">
                            Start Quiz
                        </button>
                    </form>
                @elseif ($attempt->status === 'in_progress' && $remainingSeconds !== null)
                    <div class="mt-6 rounded-xl border border-amber-100 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                    ...
                    @else
                        <form class="mt-6 space-y-5" method="POST" action="{{ route('attempt.submit', $attempt) }}">
                            ...
                            @foreach ($questions as $question)
                                <fieldset class="rounded-2xl border border-gray-200 bg-gray-50/50 p-5">

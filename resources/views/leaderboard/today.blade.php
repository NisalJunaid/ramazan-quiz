...
@section('content')
    <div class="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 sm:px-6 lg:px-8">
        <header class="flex flex-wrap items-center justify-between gap-4">
            ...
            <a class="text-sm font-semibold text-indigo-600 hover:text-indigo-700" href="{{ route('home') }}">Back to Home</a>
        </header>
        ...
        @else
            <section class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <span class="rounded-full bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-600">{{ $quizDay->quiz_date }}</span>
                </div>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach ($attempts as $index => $attempt)
                                    @php
                                        $isTop = $index < 3;
                                    @endphp
                                    <tr class="{{ $isTop ? 'bg-indigo-50/60' : 'bg-white' }}">
                                        <td class="px-4 py-3 font-semibold text-gray-900">
                                            {{ $index + 1 }}
                                        </td>
                                        <td class="px-4 py-3 {{ $isTop ? 'font-semibold text-indigo-700' : 'text-gray-700' }}">
                                            {{ $attempt->user->name }}
                                        </td>
                                        <td class="px-4 py-3 font-semibold text-gray-900">
                                            {{ $attempt->score }}
